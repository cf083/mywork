...
resp_json = resp.json()
if 'layers' not in resp_json and 'manifests' in resp_json:
	print('[+] Manifests found for this tag (use the @digest format to pull the corresponding image):')
	manifests = resp_json['manifests']
	for manifest in manifests:
		for key, value in manifest["platform"].items():
			sys.stdout.write('{}: {}, '.format(key, value))
		print('digest: {}'.format(manifest["digest"]))
...
if len(imgparts[:-1]) != 0:
	content[0]['RepoTags'].append('/'.join(imgparts[:-1]).lower() + '/' + img + ':' + tag)
else:
...
if len(imgparts[:-1]) != 0:
	content = { '/'.join(imgparts[:-1]).lower() + '/' + img : { tag : fake_layerid } }
